        ...
        if(nameErr && descriptionErr && skuErr && manufacturerErr)
        {
            if(quantityErr){
                ...
                const userFound = await User.findOne({
                    ...
                    if(err){
                        console.log(err);
                        logger.customlogger.error('DB Error: User is not found during the post of Product')    
                    }
                });
                ...
                const productFound = await Product.findOne({
                    ...
                    if(err){
                        console.log(err);
                        logger.customlogger.error('DB Error: ProductFound issue during the post of Product')  
                    }
                });
                ...
                if(productFound != null){
                    ...
                    logger.customlogger.error('SKU needs to be unique')  
                }else{
                    await Product.create({
                        ...
                        if(err){
                            console.log(err);
                            logger.customlogger.error('DB Error: Product is not created during the post of Product') 
                        }
                    });
    
                    res.status(201)
                    logger.customlogger.info('Created the Product Sucessfully') 
                    const productLoaded = await Product.findOne({
                        where: { sku: sku },
                    }).catch((err) => {
                        if(err){
                            console.log(err);                 
                            logger.customlogger.error('DB Error: Product created during the post of Product is not been able to fetch') 
                        }
                    });
                ...
                logger.customlogger.error('Please give the Quantity of Product which nedds to more than 0 and less than 100')
            }
        }else{
            res.status(400).send("Please give values to name,description,sku or manufacturer");
            logger.customlogger.error('Please give values to name,description,sku or manufacturer')
        }
    }else{
...
const GetAllProducts = async (req,res) => { 
    ...
    if(idError){
        ...
        const productFound = await Product.findOne({
            ...
            if(err){
                console.log(err);
                logger.customlogger.error('DB Error: Product is not found during the get of Product')    
            }
        });

        if(productFound == null ){
            res.status(403).send("The ProductId doesn't exists")
            logger.customlogger.error('The ProductId does not exists')
        }else{
            res.status(200);
...
const PutAllProducts = async (req,res) => {
    ...
    if(Object.keys(response).length != 0){
        ...
        if(responseErr){
                ...
                if(nameErr && descriptionErr && skuErr && manufacturerErr)
                {
                    if(quantityErr){
                        ...
                        const userFound = await User.findOne({
                            ...
                            if(err){
                                console.log(err);    
                                logger.customlogger.error('DB Error: User is not found during the Put of Product')  
                            }
                        });
                        ...
                        const productFound = await Product.findOne({
                            ...
                            if(err){
                                console.log(err);
                                logger.customlogger.error('DB Error: ProductFound issues during the Put of Product')
                            }
                        });
        
                        if(productFound == null){
                            res.status(403).send("ProductId doesn't exists");
                            logger.customlogger.error("ProductId doesn't exists")
                        }else{
                            ...
                            const productSku = await Product.findOne({
                                ...
                                if(err){
                                    console.log(err);
                                    logger.customlogger.error('DB Error: Fetching the product with sku issue')
                                }
                            });
                            ...
                            if(productSku != null && productFound.sku != productSku.sku){
                                ...
                                logger.customlogger.error('DB Error: Error Updating the Product')
                                }); 
            ...
            logger.customlogger.error("UnIntened Key is being sent")  
        }
    }
    else{
        res.status(204).send("No Content");
        logger.customlogger.error("No Content")
    }
};
            ...
            if(productIdErr){ 
                    ...
                    if(quantityErr){
                        ...
                        const userFound = await User.findOne({
                            ...
                            if(err){
                                console.log(err);
                                logger.customlogger.error('DB Error: User is not found during the Patch of Product')
                            }
                        });
                        ...
                        const productFound = await Product.findOne({
                            ...
                            if(err){
                                console.log(err);
                                logger.customlogger.error('DB Error: ProductFound issues during the Patch of Product')
                            }
                        });
        
                        if(productFound == null){
                            res.status(403).send("Product doesn't exists");
                            logger.customlogger.error("Product doesn't exists")
                        }else{
                            ...
                            if(productSku != null && productFound.sku != productSku.sku){
                                ...
                                console.error("Error updating user: ", error);
                                logger.customlogger.error('DB Error: Error Patching the Product')
                                }); 
...
const DeleteAllProducts = async (req,res) => { 
    ...
    if(idError){
        ...
        const productFoundOne = await Product.findOne({
            ...
            if(err){
                console.log(err);
                logger.customlogger.error('DB Error: ProductFound issues during the Delete of Product')
            }
        });
       ...
       if(productFoundOne != null){
        const imagesFound = await Images.findAll({
            ...
            if(err){
                console.log(err);
                logger.customlogger.error('DB Error: Image fetch issues during the Delete of Product')
            }
        });
        ...
        if(imagesFound != null ){
            ...
            await Images.destroy({
                ...
                if(err){
                    console.log(err);
                    logger.customlogger.error('DB Error: Images delete issues during the Delete of Product')
                }
            });
        }   
        ...
        const productFound =  await Product.destroy({
            ...
            if(err){
                console.log(err);
                logger.customlogger.error('DB Error: Product fetch issues during the Delete of Product')
            }
        });
